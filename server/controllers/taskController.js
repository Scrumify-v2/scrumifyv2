...
taskController.getAllTasks = async (req, res, next) => {
  ...
  try {
    const userTasks = await Task.find({ user: req.params.user });
...
/// NEED TO UPDATE PAYLOAD TO ADD USER INFO
...
taskController.moveTask = async (req, res, next) => {
  if (!req.body.taskId) {
    return next({
      ...
      status: 400,
      message:
        'Moving task/changing progress requires Task ID of task to be moved',
    });
  }
  if (!req.body.progress) {
    return next({
...
taskController.updateContent = async (req, res, next) => {
  ...
  try {
    ...
    const updatedContentTask = await Task.findByIdAndUpdate(
      taskId,
      {
        content: content,
      },
