...
projectController.getAllTasks = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.getAllTasks. ERROR: no tasks exist for this project yet ${error}`,
      },
    });
  }
};
...
projectController.createNewTask = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.createNewTask. ERROR: invalid format/type for tasks ${error}`,
      },
    });
  }
};
...
projectController.editTask = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.editTask. ERROR: task not found ${error}`,
      },
    });
  }
};
...
projectController.updateTaskProgress = async (req, res, next) => {
  try {
    ...
    const firstSearch = await Project.findOne({
      [searchToBeStarted]: { $exists: true },
    });

    // Looks to see if the task with unique ID is in the in_progress bucket
    const searchInProgress = `progress.in_progress.${_id}`;
    const secondSearch = await Project.findOne({
      [searchInProgress]: { $exists: true },
    });

    // If the task with unique ID is in to_be_started, advance it to in_progress
    if (firstSearch) {
      const firstUpdate = await Project.findOneAndUpdate(
        { [searchToBeStarted]: task },
        { [searchInProgress]: task },
        { new: true }
      );
      const firstDelete = await Project.findOneAndUpdate(
        { [searchToBeStarted]: task },
        { $unset: { [searchToBeStarted]: task } },
        { new: true }
      );
    }
    // If the task with unique ID is in in_progress, advance it to completed
      ...
      const secondUpdate = await Project.findOneAndUpdate(
        { [searchInProgress]: task },
        { [searchCompleted]: task },
        { new: true }
      );
      const secondDelete = await Project.findOneAndUpdate(
        { [searchInProgress]: task },
        { $unset: { [searchInProgress]: task } },
        { new: true }
      );
    } else {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.updateTaskProgress. ERROR: task not found ${error}`,
      },
    });
  }
};
...
projectController.deleteTask = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.deleteTask. ERROR: task not found ${error}`,
      },
    });
  }
};
...
projectController.getProject = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.getProject. ERROR: invalid format for project ${error}`,
      },
    });
  }
};
...
projectController.createNewProject = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.createNewProject. ERROR: invalid format for project ${error}`,
      },
    });
  }
};
...
projectController.deleteProject = async (req, res, next) => {
  try {
    ...
    return next({
      ...
      message: {
        err: `Error has occured in projectController.deleteProject. ERROR: project not found ${error}`,
      },
    });
  }
};
